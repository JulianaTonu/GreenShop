    ...
    return product && (
        <div className="mt-12">
            <p>
                ...
                <Link to={`/products/${product.category.toLowerCase()}/${product._id}`}> {product.category}</Link> /
                ...
                <div className="flex gap-3">
                    <div className="flex flex-col gap-3">
                        {product.image.map((image, index) => (
                            <div key={index} onClick={() => setThumbnail(image)} className="border max-w-24 border-gray-500/30 hover:bg-primary/10 rounded overflow-hidden cursor-pointer" >
                                <img src={image} alt={`Thumbnail ${index + 1}`} />
                            </div>
                        ))}
                    </div>

                    <div className="border border-gray-500/30 max-w-100 rounded overflow-hidden hover:bg-primary/10">
                    ...
                    <div className="flex items-center gap-0.5 mt-1">
                        {Array(5).fill('').map((_, i) => (
                            ...
                            <img src={i < 4 ? assets.star_icon : assets.star_dull_icon}
                                className='md:w-4 w-3.5' alt="" srcset="" />
            ...
            md:gap-6 xl:grid-cols-5 '>
                ...
                <button className='mx-auto cursor-pointer px-12 my-16 py-2.5 border rounded
                 text-primary  hover:bg-primary/30 transition'>See More</button>
            </div>
